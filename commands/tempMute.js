...
module.exports.run = async(bot, message, args) => {
    ...
    if (!message.member.hasPermission("KICK_MEMBER")) return message.reply("sorry jij kan dit niet gebruiken");

    if (!args[1]) return message.reply("geen gebruiker opgegeven.");

    if (!message.guild.me.hasPermission("KICK_MEMBER")) return message.reply("geen perms");
